...
def train_BERT(args):
    ...
    for epoch in range(args.epochs):
        ...
        MCC = (TP.item() * TN.item() - FP.item() * FN.item()) / (
                    (TP.item() + FP.item()) * (TP.item() + FN.item()) * (TN.item() + FP.item()) * (
                        TN.item() + FN.item())) ** 0.5 if ((TP.item() + FP.item()) * (TP.item() + FN.item()) * (
                    TN.item() + FP.item()) * (TN.item() + FN.item())) != 0 else 0
        ...
        pd.DataFrame(result,
                     columns=['epoch', 'val_loss', 'val_acc', 'val_pre', 'val_rec', 'val_f_score', 'val_MCC']).to_csv(
...
def train_UniAMP(args):
    if args.feature == 'pca':
        ...
        model = UniAMP(1900, args)
        data = np.load(args.dataset_path, allow_pickle=True)
        X_train = np.array([data[key].flatten()[0]['avg'] for key in data.keys()])
        Y_train = np.array([float(key[-1]) for key in data.keys()])
        callbacks_list = [EarlyStopping(monitor='val_get_MCC', patience=args.patience, mode='max'),
                          ModelCheckpoint(
                              filepath=os.path.join(args.save_dir, 'model_UniAMP_uni.h5'),
                              monitor='val_get_MCC', mode='max', save_best_only=False),
                          CSVLogger(os.path.join(args.log_dir, 'log_UniAMP_uni.csv'), separator=',', append=False),
                          ]
        ...
        callbacks_list = [EarlyStopping(monitor='val_get_MCC', patience=args.patience, mode='max'),
                          ModelCheckpoint(
                              ...
                              monitor='val_get_MCC', mode='max', save_best_only=True),
                          CSVLogger(os.path.join(args.log_dir, f'log_UniAMP_comparison_{args.comparison}.csv'),
